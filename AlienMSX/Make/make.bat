	...
	for /F "tokens=*" %%A in (%MSX_CFG_PATH%\LibrarySources.txt) do (
		...
		if NOT "%LIBFILE:~0,1%"==";" (
			...
			if /I "%%~xA"==".c" (
				...
				REM call :exec %DBG_CALL2% sdcc --sdcccall %SDCC_CALL% %SDCC_DETAIL% %COMPILER_EXTRA_DIRECTIVES% -mz80 -c %INCDIRS% -o "!RELFILE!" "!LIBFILE!"
...
:compile
	...
	for /F "tokens=1" %%A in  (%MSX_CFG_PATH%\ApplicationSources.txt) do  (
		...
		if NOT "%APPFILE:~0,1%"==";" (
			...
			if /I "%%~xA"==".c" (
				...
				REM call :exec %DBG_CALL2% sdcc --sdcccall %SDCC_CALL% %SDCC_DETAIL% %COMPILER_EXTRA_DIRECTIVES% -mz80 -c %INCDIRS% -o "!RELFILE!" "!APPFILE!"
	...
	call :exec %DBG_CALL2% sdcc %SDCC_DETAIL% %LINKER_EXTRA_DIRECTIVES% --code-loc %CODE_LOC% --data-loc %DATA_LOC% -mz80 --no-std-crt0 %OBJLIST% %INCDIRS% -o "%MSX_OBJ_PATH%\%MSX_FILE_NAME%.IHX"
...
:build_msx_bin
	...
	if ".%BIN_SIZE%"=="." (
		call :exec %DBG_CALL2% hex2bin %HEX2BIN_DETAIL% %EXECGEN_EXTRA_DIRECTIVES% -e %MSX_FILE_EXTENSION% "%MSX_OBJ_PATH%\%MSX_FILE_NAME%.IHX"
	) else (
		call :exec %DBG_CALL2% hex2bin %HEX2BIN_DETAIL% %EXECGEN_EXTRA_DIRECTIVES% -e %MSX_FILE_EXTENSION% -l %BIN_SIZE% "%MSX_OBJ_PATH%\%MSX_FILE_NAME%.IHX"
	)
	call :exec %DBG_EXTROVERT% python Make\chksize.py %BIN_SIZE% %DATA_LOC% "%MSX_OBJ_PATH%\%MSX_FILE_NAME%.MAP"
