...
def zx0_compress(data):
    with tempfile.NamedTemporaryFile() as fd:
        ...
        ap_name = fd.name + ".zx0"
        subprocess.call(["zx0", fd.name, ap_name], stdout=sys.stderr)
...
def main():
    ...
    if args.aplib and args.zx0:
        parser.error("Can't compress same map twice. Choose APLIB or ZX0")
    ...
    print("/* compressed: %s %s */" % args.aplib %args.zx0)
